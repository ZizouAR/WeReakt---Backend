...
router.post('/create', validator_1.default(schema_1.default.insert), asyncHandler_1.default(async (req, res) => {
    const job = await JobRepo_1.default.create(req.body);
    new ApiResponse_1.SuccessResponse('created', job).send(res);
}));
router.get('/departement/:id', validator_1.default(schema_1.default.identifier, validator_1.ValidationSource.PARAM), asyncHandler_1.default(async (req, res) => {
    const jobs = await JobRepo_1.default.findByDepartement(new mongoose_1.Types.ObjectId(req.params.id));
    new ApiResponse_1.SuccessResponse('retreived', jobs).send(res);
}));
router.get('/network/:id', validator_1.default(schema_1.default.identifier, validator_1.ValidationSource.PARAM), asyncHandler_1.default(async (req, res) => {
    const jobs = await JobRepo_1.default.findByNetwork(new mongoose_1.Types.ObjectId(req.params.id));
    new ApiResponse_1.SuccessResponse('retreived', jobs).send(res);
}));
router.delete('/remove/:id', validator_1.default(schema_1.default.identifier, validator_1.ValidationSource.PARAM), asyncHandler_1.default(async (req, res) => {
    const job = await JobRepo_1.default.remove(new mongoose_1.Types.ObjectId(req.params.id));
    new ApiResponse_1.SuccessResponse('removed', job).send(res);
}));
